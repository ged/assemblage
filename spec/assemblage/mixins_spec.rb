...
describe Assemblage, "mixins" do
	...
	describe Assemblage::DataUtilities do

		it "doesn't try to dup immediate objects" do
			expect( Assemblage::DataUtilities.deep_copy( nil ) ).to be( nil )
			expect( Assemblage::DataUtilities.deep_copy( 112 ) ).to be( 112 )
			expect( Assemblage::DataUtilities.deep_copy( true ) ).to be( true )
			expect( Assemblage::DataUtilities.deep_copy( false ) ).to be( false )
			expect( Assemblage::DataUtilities.deep_copy( :a_symbol ) ).to be( :a_symbol )
		end


		it "doesn't try to dup modules/classes" do
			klass = Class.new
			expect( Assemblage::DataUtilities.deep_copy( klass ) ).to be( klass )
		end


		it "doesn't try to dup IOs" do
			data = [ $stdin ]
			expect( Assemblage::DataUtilities.deep_copy( data[0] ) ).to be( $stdin )
		end


		it "doesn't try to dup Tempfiles" do
			data = Tempfile.new( 'strelka_deepcopy.XXXXX' )
			expect( Assemblage::DataUtilities.deep_copy( data ) ).to be( data )
		end
		...
		it "makes distinct copies of arrays and their members" do
			...
			copy = Assemblage::DataUtilities.deep_copy( original )
